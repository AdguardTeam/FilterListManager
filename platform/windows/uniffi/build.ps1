    ...
    foreach ($arch in $ARCH_TO_FOLDER_MAP.Keys) {
       foreach ($file in $FILES) {     
        ...
        foreach ($config in $CONFIGS) {
            $destPath = "platform\windows\uniffi\build\bin\$config\$folder\$file"
...
function SetAdapterVersion {
    ...
    $jsonFilePath = "platform\windows\uniffi\AdGuard.FilterListManager\AdGuard.FilterListManager.schema.json";
    ...
    $csprojFilePath = "platform\windows\uniffi\AdGuard.FilterListManager\AdGuard.FilterListManager.csproj";
