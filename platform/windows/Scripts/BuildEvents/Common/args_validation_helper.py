...
# Decorator implementation taken from https://stackoverflow.com/a/37961120, only name changed
if __debug__:
    import inspect
    from functools import wraps
    from inspect import Parameter, Signature

    def check_arguments_type(func: callable) -> callable:
        '''
        ...
        * Tuples of classes (e.g., `(int, OrderedDict)`).
        ...
        ----------
        NameError
            If any parameter has the reserved name `__check_arguments_type_func`.
        TypeError
            If either:
        ...
        # * A private "__check_arguments_type_func" parameter initialized to this function.
        ...
        func_body = '''
@wraps(__check_arguments_type_func)
def func_check_arguments_type(*args, __check_arguments_type_func=__check_arguments_type_func, **kwargs):
'''
        ...
        for func_arg_index, func_arg in enumerate(func_sig.parameters.values()):
            ...
            if func_arg.name == '__check_arguments_type_func':
                raise NameError(
                    'Parameter {} reserved for use by @check_arguments_type.'.format(
                        func_arg.name))
            ...
            if (func_arg.annotation is not Parameter.empty and
                ...
                func_arg_type_expr = (
                    '__check_arguments_type_func.__annotations__[{!r}]'.format(
                        func_arg.name))
...
'''.format(
        ...
        if func_sig.return_annotation not in _RETURN_ANNOTATION_IGNORED:
            ...
            func_return_type_expr = (
                "__check_arguments_type_func.__annotations__['return']")

            # Call this callable, type check the returned value, and return this
            # value from this wrapper.
            func_body += '''
    return_value = __check_arguments_type_func(*args, **kwargs)
...
'''.format(func_name=func_name, return_type=func_return_type_expr)
        # Else, call this callable and return this value from this wrapper.
            ...
            func_body += '''
    return __check_arguments_type_func(*args, **kwargs)
'''
        ...
        local_attrs = {'__check_arguments_type_func': func}
        ...
        # Return this wrapper.
        return local_attrs['func_check_arguments_type']
    ...
    annotation- based type checking in the `@check_arguments_type` decorator.
    ...
    * Constants specific to positional-only parameters, which apply to non-pure-
      Python callables (e.g., defined by C extensions). The `@check_arguments_type`
    ...
    based type checking in the `@check_arguments_type` decorator.
    ...
    * `None`, signifying a callable returning no value. By convention, callables
    ...
    def _check_type_annotation(annotation: object, label: str) -> None:
        ...
        `@check_arguments_type` decorator.
...
def raise_if_empty(argument_name, argument_value, logger):
    """Raises error if argument value is null or empty"""
    if(argument_value.isspace() or argument_value is None ):
         function_name = inspect.stack()[-1] 
         logger.info(f"{function_name} function: argument {argument_name} is null or empty")
         raise ArgumentError