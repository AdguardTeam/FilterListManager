...
public struct FilterListManager_AGOuterError {
  ...
  public enum OneOf_Error: Equatable {
    ...
    public static func ==(lhs: FilterListManager_AGOuterError.OneOf_Error, rhs: FilterListManager_AGOuterError.OneOf_Error) -> Bool {
      ...
      case (.invalidConfiguration, .invalidConfiguration): return {
        guard case .invalidConfiguration(let l) = lhs, case .invalidConfiguration(let r) = rhs else { preconditionFailure() }
        return l == r
      }()
...
/// Do not duplicate the message
public struct FilterListManager_InvalidConfiguration {
  // SwiftProtobuf.Message conformance is added in an extension below. See the
  // `Message` and `Message+*Additions` files in the SwiftProtobuf library for
  // methods supported on all messages.
...
extension FilterListManager_InvalidConfiguration: @unchecked Sendable {}
