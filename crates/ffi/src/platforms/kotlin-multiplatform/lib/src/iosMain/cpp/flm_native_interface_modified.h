...
/*
 * IMPORTANT: Modified copy for Kotlin/Native cinterop compatibility
 *
 * This file is a manually modified copy of the original flm_native_interface.h
 * to work around a bug in Kotlin/Native cinterop tool.
 *
 * PROBLEM:
 * The original header contains a pattern that causes Kotlin/Native cinterop to fail:
 *   enum RustResponseType { ... };
 *   #ifndef __cplusplus
 *   typedef uint8_t RustResponseType;  // <- This causes "going to be declared twice" error
 *   #endif
 *
 * The cinterop tool sees both the enum declaration and the typedef with the same name
 * and tries to generate two Kotlin types with identical names, resulting in:
 *   IllegalArgumentException: 'RustResponseType' is going to be declared twice
 *
 * SOLUTION:
 * We removed the problematic typedef and kept only the enum declaration with explicit
 * base type specification:
 *   typedef enum RustResponseType : uint8_t { ... } RustResponseType;
 *
 * MAINTENANCE:
 * This file must be manually updated whenever the original flm_native_interface.h changes.
 * Copy the original file and apply the same RustResponseType enum modification.
 */
