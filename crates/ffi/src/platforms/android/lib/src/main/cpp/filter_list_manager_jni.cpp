...
static jobject rust_response_to_jobject(JNIEnv *env, RustResponse *response) {
    // Find the NativeInterface class
    jclass nativeInterfaceClass = env->FindClass("com/adguard/flm/jni/NativeInterface");
    if (nativeInterfaceClass == nullptr) {
        return nullptr; // Error is set by FindClass
    }

    // Find the RustResponse inner class
    jclass responseClass = env->FindClass("com/adguard/flm/jni/RustResponse");
    if (responseClass == nullptr) {
        return nullptr; // Error is set by FindClass
    }

    // Find the RustResponseType enum class
    jclass responseTypeClass = env->FindClass("com/adguard/flm/jni/RustResponseType");
    ...
    jfieldID rustBufferField = env->GetStaticFieldID(responseTypeClass, "RustBuffer",
                                                     "Lcom/adguard/flm/jni/RustResponseType;");
    ...
    jmethodID constructor = env->GetMethodID(responseClass, "<init>",
                                             "(JLjava/nio/ByteBuffer;ZJLcom/adguard/flm/jni/RustResponseType;)V");
...
Java_com_adguard_flm_jni_NativeInterface_flmDefaultConfigurationProtobuf(JNIEnv *env, jclass clazz) {
...
extern "C"
JNIEXPORT jobject JNICALL
Java_com_adguard_flm_jni_NativeInterface_flmInitProtobuf(JNIEnv *env, jclass clazz, jbyteArray bytes) {
...
Java_com_adguard_flm_jni_NativeInterface_flmFreeHandle(JNIEnv *env, jclass clazz, jlong handle) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_adguard_flm_jni_NativeInterface_flmFreeResponse(JNIEnv *env, jclass clazz, jlong handle) {
    // Call the C function to free the response
    flm_free_response(reinterpret_cast<RustResponse *>(handle));
}

extern "C"
JNIEXPORT jobject JNICALL
Java_com_adguard_flm_jni_NativeInterface_flmGetConstants(JNIEnv *env, jclass clazz) {
    ...
    jclass constantsClass = env->FindClass(
            "com/adguard/flm/FilterListManagerConstants");
