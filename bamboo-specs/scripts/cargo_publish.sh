...
echo "ADGUARD_FLM_VERSION: $ADGUARD_FLM_VERSION"
...
FOUND=$(cargo search adguard-flm | grep "adguard-flm = \"$ADGUARD_FLM_VERSION\"" || true)
...
if [ -z "$FOUND" ]; then
  ...
  FOUND=$(cargo search $CRATE_NAME | grep "$CRATE_NAME = \"$ADGUARD_FLM_VERSION\"" || true)
  ATTEMPT_COUNT=0
  ATTEMPT_LIMIT=20
  SLEEP_TIME=10
...
FFI_FOUND=$(cargo search ffi | grep "ffi = \"$FFI_VERSION\"" || true)
